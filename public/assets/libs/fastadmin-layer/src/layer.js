...
Class.pt.vessel = function(conType, callback){
  var that = this, times = that.login, config = that.config;
...
Class.pt.creat = function(){
  ...
  //遮罩
  $('#layui-layer-shade'+ that.login).css({
...
Class.pt.callback = function(){
  ...
  if(config.success){
    if(config.type == 2){
      layero.find('iframe').on('load', function(){
        config.success(layero, that.login);
      });
    } else {
      config.success(layero, that.login);
    }
  ...
  layero.find('.'+ doms[6]).children('a').on('click', function(){
    ...
    if(index === 0){
      if(config.yes){
        config.yes(that.login, layero)
      } else if(config['btn1']){
        config['btn1'](that.login, layero)
      } else {
        layer.close(that.login);
      }
    } else {
      var close = config['btn'+(index+1)] && config['btn'+(index+1)](that.login, layero);
      close === false || layer.close(that.login);
    }
  });
  ...
  //取消
  function cancel(){
    var close = config.cancel && config.cancel(that.login, layero);
    close === false || layer.close(that.login);
  }
  ...
  if(config.shadeClose){
    $('#layui-layer-shade'+ that.login).on('click', function(){
      layer.close(that.login);
    });
  } 
  ...
  layero.find('.layui-layer-min').on('click', function(){
    ...
    min === false || layer.min(that.login, config);
  });
  
  //全屏/还原
  layero.find('.layui-layer-max').on('click', function(){
    if($(this).hasClass('layui-layer-maxmin')){
      layer.restore(that.login);
      config.restore && config.restore(layero);
    } else {
      layer.full(that.login, config);
      setTimeout(function(){
  ...
  config.end && (ready.end[that.login] = config.end);
};
